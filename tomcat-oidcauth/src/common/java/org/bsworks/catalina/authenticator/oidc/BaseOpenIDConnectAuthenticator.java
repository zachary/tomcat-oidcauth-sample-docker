...
public abstract class BaseOpenIDConnectAuthenticator
	...
	/**
	 * Callback (form login) action URL path. Must start with a "/" and be
	 * relative to the web-application context. Defaults to
	 * {@link Constants#FORM_ACTION}.
	 */
	...
	/**
	 * Get callback (form login) action URL path.
	 *
	 * @return Callback path, or {@code null} if not set.
	 */
	...
	/**
	 * Set callback (form login) action URL path. This is the path the
	 * OpenID Connect Provider redirects back to after authentication. Must
	 * start with a "/" and be relative to the web-application context (the
	 * web-application's context path is prepended automatically). Defaults to
	 * {@code /j_security_check} ({@link Constants#FORM_ACTION}) if not set.
	 *
	...
	protected boolean performAuthentication(final Request request,
		...
		final boolean loginAction = (
				...
				requestURI.endsWith(this.getCallbackAction()));
	...
	/**
	 * Restore the original request from the session after successful
	 * authentication and tell if the restore was successful.
	 *
	 * @param request The request.
	 * @param session The session.
	 *
	 * @return {@code true} if the original request was restored successfully.
	 *
	 * @throws IOException If an I/O error happens.
	 */
	protected abstract boolean restoreOriginalRequest(final Request request,
	...
	protected boolean processResubmit(final Request request,
		...
		if (!this.restoreOriginalRequest(request, session)) {
			if (debug)
	...
	protected void addLoginConfiguration(final Request request)
		...
		for (int i = 0; i < this.opDescs.size(); i++) {
			...
			buf.append("&redirect_uri=").append(URLEncoder.encode(
					this.getBaseURL(request) + this.getCallbackAction(),
					UTF8.name()));
	...
	protected TokenEndpointResponse callTokenEndpoint(final OPDescriptor opDesc,
		...
		buf.append("&redirect_uri=").append(URLEncoder.encode(
				this.getBaseURL(request) + this.getCallbackAction(), UTF8.name()));
	 ...
	 * Get callback (form login) action URL path.
	 *
	 * @return Callback path, or {@link Constants#FORM_ACTION} if not set.
	 */
	protected String getCallbackAction() {
		...
		return Constants.FORM_ACTION;
	}

	/**
	 * Tell if the request requires authentication continuation. Overridden to
	 * recognize the configured callback path (in addition to the default
	 * {@link Constants#FORM_ACTION}) as an authentication submission, so the
	 * OpenID Connect Provider redirect is processed by the authenticator even
	 * though the callback URL is not covered by a security constraint.
	 *
	 * @param request The request.
	 *
	 * @return {@code true} if authentication continuation is required.
	 */
	@Override
